...
import { EXCHANGE_RATE, getProvider, UG_PREFIX } from '@/lib/constants';
...
export const WalletModal: React.FC<WalletModalProps> = ({ isOpen, onClose, onDepositSuccess, phone, balance = 0 }) => {
  ...
  const handleAction = async () => {
    ...
    try {
      if (mode === 'DEPOSIT') {
          const result = await walletService.deposit(finalAmount);
          ...
          const result = await walletService.withdraw(finalAmount);
          ...
          setStep('SUCCESS');
          setPendingDeposit(null);
          const newBal = (result && typeof result.balance === 'number') ? result.balance : undefined;
      ...
      setLoading(false);
      setStep('SELECT');
      const message = e instanceof Error ? e.message : 'Transaction failed. Please try again.';
      setError(message);
    }
  };
