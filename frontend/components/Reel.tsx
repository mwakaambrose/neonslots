...
import { SYMBOLS, REEL_SEQUENCE } from '../constants';

interface ReelProps {
  symbol: SymbolType;
  isSpinning: boolean;
  stopDelay: number; // Delay in ms before this specific reel stops
  onStop: () => void;
  highlight: boolean;
}
...
export const Reel: React.FC<ReelProps> = ({ symbol, isSpinning, stopDelay, onStop, highlight }) => {
  ...
  useEffect(() => {
    if (isSpinning) {
      ...
      }, 80); // 80ms per frame = fast spin

      // Schedule the stop
      const timeoutId = setTimeout(() => {
        if (intervalRef.current) clearInterval(intervalRef.current);
        setDisplaySymbol(symbol); // Snap to result
        setBlur(false);
      ...
      return () => {
        ...
        // If not spinning (e.g. init), ensure symbol matches prop
